...
class GeneralCounterMeasure:
    def __init__(self, simulation, poi_category, closing_hour=0):
    ...
    def apply(self, simulation_time, original_hour_movement_table):
        mask = self.simulation.poi_categories == self.poi_category
    ...
    def end_simulation(self):
        sector_fd = self.simulation.sectors_graph.get_edge_initial_exchange_total(self.poi_category)
        sector_loss = (self.supposed_people_inside_during_closing / self.total_people_inside_without_changes) * sector_fd
        return [(self.poi_category, sector_loss)]
