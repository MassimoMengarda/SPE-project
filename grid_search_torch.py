...
def main(simulation_parameters_filepath, info_dir, ipfp_dir, dwell_dir, sector_graph_filepath, cases_filepath, output_dir):
    io_handle, counter_measure_file_path = tempfile.mkstemp(prefix="counter_measure", dir=None, text=True)
    with os.fdopen(io_handle, "w") as f:
        f.write("\n")
    ...
    with torch.no_grad():
        ...
        for i, line in parameters_df.iterrows():
            ...
            m = Model(cbgs_population, ipfp_dir, dwell_dir, torch.from_numpy(poi_categories["io_sector"].to_numpy()).cuda(), sector_graph_filepath, counter_measure_file_path, n_pois, pois_area, b_bases=b_bases_torch, psis=psis_torch, p_0s=p_0s_torch, t_e=96, t_i=84, batch=rows_per_simulation * batch)
            ...
            for simulation_time, week_string, week_t, cbg_s, cbg_e, cbg_i, cbg_r_dead, cbg_r_alive, cbg_new_i, diff_visit_countermeasures in m.simulate(simulation_start, simulation_end):
                ...
                if simulation_time.hour == 0:
                    ...
                    if cases_index in cases.index:
                        cumulative_cases = cases.loc[cases_index]["cases"]
                        certified_new_cases = cumulative_cases - previous_day_cumulative_cases
                        previous_day_cumulative_cases = cumulative_cases
                    else:
...
if __name__ == "__main__":
    ...
    main(simulation_parameters_filepath, info_dir, ipfp_dir, delta_pj_dir, sector_graph_filepath, cases_filepath, output_dir)
